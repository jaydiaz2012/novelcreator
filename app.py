...
with st.sidebar:
    ...
    # Story Variables
    genre = st.selectbox(
        "Genre", ["Techno-Thriller", "Cyberpunk", "High Fantasy", "Romance"]
    )
    tone = st.select_slider(
        "Tone Intensity", options=["Light", "Balanced", "Gritty", "Dark"]
    )
...
def write_draft(scene_card, style):
    ...
    long_sentence = (
        "He didn't panic. He never panicked. He just typed faster, "
        "his fingers blurring like hummingbirds against the mechanical keys."
    )
    return (
        "\n"
        "    The neon lights of the server room hummed with a headache-inducing buzz.\n"
        f"    This was the style of {style}—short, punchy, no nonsense.\n"
        "\n"
        '    "System breach," the console read.\n'
        "\n"
        f"    {long_sentence}\n"
        f"    (This is a mock draft generated based on: {scene_card})\n"
        "    "
    )
...
with col1:
    ...
    premise_input = st.text_area(
        "Enter Chapter Premise:",
        height=100,
        placeholder="e.g., Jinx hacks the mainframe but triggers a trap.",
    )
    ...
    if st.button("🏗️ Generate Outline"):
        with st.spinner("Architect is thinking..."):
            ...
            st.session_state["scene_card"] = generate_outline(
                premise_input, genre, bible_context
            )
            st.rerun()
    ...
    if st.session_state["scene_card"]:
        ...
        edited_card = st.text_area(
            "Scene Card (JSON/XML)", value=st.session_state["scene_card"], height=300
        )
        st.session_state["scene_card"] = edited_card

        if st.button("✍️ Write Draft"):
            with st.spinner("Drafter is writing..."):
                st.session_state["draft"] = write_draft(
                    st.session_state["scene_card"], style
                )
                st.rerun()
    ...
    if st.session_state["draft"]:
        ...
        if st.button("🧐 Analyze & Edit"):
            with st.spinner("Editor is reviewing..."):
                st.session_state["report"] = critique_draft(
                    st.session_state["draft"], genre
                )

                # Mock Archivist Update (In real app, use LLM here)
                st.session_state["story_bible"][
                    "summary"
                ] += f" -> Processed premise: {premise_input}"
                st.rerun()
